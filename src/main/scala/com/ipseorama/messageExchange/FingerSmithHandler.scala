...
class FingerSmithHandler extends Actor {
  ...
  private def writeHTML(ctx: HttpRequestEvent) {
    ...
    buf.append("<body>\n")
    buf.append("<script src=\"http://code.jquery.com/jquery-1.8.3.js\"></script>")
    buf.append("<script src=\"/js/phono.sdp.js\"></script>\n")
    buf.append("<script src=\"/js/ipseorama.js\"></script>\n")
    buf.append("<script type=\"text/javascript\">\n")
    buf.append("  var socket;\n")
    buf.append("  function gotFinger(finger){\n")
    buf.append("  if (!window.WebSocket) {\n")
    buf.append("    window.WebSocket = window.MozWebSocket;\n")
    buf.append("  }\n")
    buf.append("  if (window.WebSocket) {\n")
    buf.append("    socket = new WebSocket(\"ws://localhost:8888/websocket/?finger=\"+finger);\n") // Note the address must match the route
    ...
    buf.append("  }\n")
    buf.append("  }\n")
    buf.append("  Ipseorama.whoAmI(gotFinger,function(err) {\nconsole.log(\"could not create identity \" + err)\n});\n")
    ...
    buf.append("  <input type=\"text\" name=\"to\" value=\"78A18BA5ED6E69E9F493AE69C8848714A48ECD487D25C24F13E8B781E55B374E\"/>\n")
    buf.append("  <input type=\"button\" value=\"Chat\" onclick=\"var m = {to:this.form.to.value, message:this.form.message.value}; send(JSON.stringify(m))\" />\n")
    buf.append("  \n")
