...
case class WebSocketHandshakeEvent(
  ...
  val finger = this.request.endPoint.getQueryString("finger")
  val webSocketId:String = finger match {
    case Some(finger) => finger
    case None => UUID.randomUUID().toString() + "-" + this.hashCode()
  }
